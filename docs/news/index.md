...
- [`ggclusternet_pq()`](https://adrientaudiere.github.io/netaipq/reference/ggclusternet_pq.md)
  now drops samples with a missing value in the `group` column before
  building the network (reporting how many were dropped), fixing an
  error (“Component sample names do not match”) raised when `group`
  contained `NA`,
  e.g. `ggclusternet_pq(data_fungi_mini, group = "Height")`.
